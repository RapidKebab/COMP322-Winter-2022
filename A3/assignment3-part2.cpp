...
public:
    ...
    void announce(Status status, int i);
};
...
void HumanPlayer::announce(Status status, int i)
{
    if (status == WIN)
    {
...
Status BlackJackGame::play()
{
    ...
    // create player
    cout << "How many players(1-3) do you wish to have?: " << endl;
    int x;
    std::cin >> x;
    HumanPlayer* hp[x];
    for (int i=0; i<x; i++) {
        hp[i] = new HumanPlayer();
    }
    cout << "Playing with " <<x<<" player(s)..." << endl;
    

    // Player is dealt 2 open card.
    for (int i=0; i<x; i++){
        (*hp[i]).hand.add(m_deck.deal());
        (*hp[i]).hand.add(m_deck.deal());
        cout << "Player "<< i<< ":";
        (*hp[i]).displayHand();
        cout << endl;
    ...
    if(bustcount==x){
        ...
        endRound();
        (*hp[x]).announce(LOST,x);
        return LOST;
    }
    //winning player determination
    HumanPlayer *player = hp[0];
    int winningPlayer = 0;
    bool tieOverride = false;
    if(x>1){
        int j=0;
        while((*hp[j]).isBusted()&&j<x){//makes sure the initial winningplayer is not busted
            j++;
            *player = *hp[j];
            winningPlayer = j;
        }
        ...
        for(int i = 0;i<x;i++){
            if((*hp[i]).hand.getTotal()<22 && i != winningPlayer && (*hp[i]).hand.getTotal()>=(*player).hand.getTotal()){
                if((*hp[i]).hand.getTotal()==(*player).hand.getTotal()){
                    tieOverride = true;
                }
                else{
                    *player = *hp[i];
    ...
    {
        if(!tieOverride){
            (*player).announce(WIN,winningPlayer);
            endRound();
            return WIN;
        }
        else{
            (*player).announce(TIE,winningPlayer);
            endRound();
            return TIE;
        }
    }
    else
    {
        ...
        {
            (*player).announce(LOST,winningPlayer);
            endRound();
            return LOST;
        }
